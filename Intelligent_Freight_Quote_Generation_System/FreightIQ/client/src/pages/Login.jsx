    ...
    try {
      ...
      if (response.ok) {
        ...
        if (backendRole === 'ADMIN') {
          role = 'admin'
        } else if (['BROKER', 'SENIOR_BROKER', 'PRICING_MANAGER', 'OPERATIONS'].includes(backendRole)) {
          role = 'broker'
        }
        localStorage.setItem('userRole', role)
        localStorage.setItem('userName', data.user.full_name || 'Agent')
        ...
        setIsLoading(false)
        const errMsg = data.detail || (data.non_field_errors && data.non_field_errors[0]) || 'Invalid credentials. Please try again.'
        alert(typeof errMsg === 'object' ? JSON.stringify(errMsg) : errMsg)
      }
    } catch (error) {
      setIsLoading(false)
      console.error('Login error:', error)
      alert('Failed to connect to the authentication server. Please ensure the backend is running.')
    }
  }
              ...
              <>
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3 mb-6 space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      { role: 'Admin', email: 'admin@freighthub.com', pwd: 'adminpassword' },
                      { role: 'Broker', email: 'broker@freighthub.com', pwd: 'brokerpassword' },
                      { role: 'User', email: 'user@freighthub.com', pwd: 'userpassword' }
                    ].map((p) => (
                      <button
                        key={p.role}
                        type="button"
