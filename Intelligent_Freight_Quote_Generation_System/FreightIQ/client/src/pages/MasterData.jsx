  ...
  useEffect(() => {
    ...
    const loadAllData = async () => {
      ...
      const endpointMap = {
        ...
        carriers: '/api/v1/masterdata/carriers/',
        serviceTypes: '/api/v1/masterdata/services/',
        containerTypes: '/api/v1/masterdata/container-types/',
        commodities: '/api/v1/masterdata/commodities/',
        currencies: '/api/v1/masterdata/currencies/',
        rateCards: '/api/v1/pricing/rate-cards/',
        surchargeRules: '/api/v1/pricing/surcharges/',
      };

      for (const col of COLLECTIONS_CONFIG) {
        const url = endpointMap[col.id];
        if (url) {
          try {
            ...
            if (response.ok) {
              ...
              const itemsList = Array.isArray(resData) ? resData : (resData.results || resData.data || []);
              
              // Normalize field name mappings
              const normalized = itemsList.map(item => {
                const norm = { ...item };
                
                // Ports mapping
                if (col.id === 'ports') {
                  norm.UNLOCODE = item.un_locode || item.UNLOCODE;
                  norm.portName = item.name || item.portName;
                  norm.portType = item.type || item.portType || 'Sea';
                  norm.iataCode = item.iata_code || item.iataCode || '';
                  norm.countryCode = item.country_id || item.countryCode || '';
                  norm.location = item.latitude && item.longitude ? `${item.latitude}, ${item.longitude}` : (item.location || '');
                  norm.timezone = item.timezone || '';
                }
                
                // Carriers mapping
                if (col.id === 'carriers') {
                  norm.carrierCode = item.code || item.carrierCode;
                  norm.carrierName = item.name || item.carrierName;
                  norm.reliabilityScore = item.reliability_rating || item.reliabilityScore || 95;
                  norm.contractTier = item.contract_tier || item.contractTier || 'Tier 1';
                }
                
                // ServiceTypes mapping
                if (col.id === 'serviceTypes') {
                  norm.serviceCode = item.code || item.serviceCode;
                  norm.serviceName = item.name || item.serviceName;
                  norm.transportMode = item.transport_mode || item.transportMode;
                }
                
                // ContainerTypes mapping
                if (col.id === 'containerTypes') {
                  norm.containerCode = item.code || item.containerCode;
                  norm.containerName = item.name || item.containerName;
                  norm.lengthFt = item.length_ft || item.lengthFt;
                  norm.widthFt = item.width_ft || item.widthFt;
                  norm.heightFt = item.height_ft || item.heightFt;
                  norm.maxPayloadKg = item.max_payload_kg || item.maxPayloadKg;
                  norm.tareWeightKg = item.tare_weight_kg || item.tareWeightKg;
                }
                
                // Commodities mapping
                if (col.id === 'commodities') {
                  norm.hsCode = item.hs_code || item.hsCode;
                  norm.commodityName = item.name || item.commodityName;
                  norm.dutyRatePct = item.duty_rate_pct || item.dutyRatePct || 10.0;
                }
                
                // Currencies mapping
                if (col.id === 'currencies') {
                  norm.currencyCode = item.code || item.currencyCode;
                  norm.currencyName = item.name || item.currencyName;
                  norm.exchangeRateToUSD = item.exchange_rate_to_usd || item.exchangeRateToUSD || 1.0;
                }

                // RateCards mapping
                if (col.id === 'rateCards') {
                  norm.rateCardId = item.id || item.rateCardId;
                  norm.baseRate = item.base_rate || item.baseRate;
                }

                return norm;
              });

              loadedData[col.id] = normalized;
              continue;
            }
          } catch (e) {
            console.warn(`Failed to fetch masterdata for ${col.id}, using local storage fallback:`, e);
          }
        }
        
        // Fallback to local storage if endpoint doesn't exist or failed
