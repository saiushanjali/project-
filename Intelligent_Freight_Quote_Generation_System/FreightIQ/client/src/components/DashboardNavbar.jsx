...
export default function DashboardNavbar({ setIsMobileOpen, title }) {
  ...
  const [notifications, setNotifications] = useState([
    { id: 'ALT-1', text: 'Monsoon weather swell advisory active across INMAA corridor', time: '10 mins ago', category: 'WEATHER' },
    { id: 'ALT-2', text: 'Customs ICEGATE BOE document required for Quote #IQ-9820', time: '1 hr ago', category: 'CUSTOMS' },
    { id: 'ALT-3', text: 'Scikit-Learn ML pricing regressor updated (R2: 0.965)', time: '3 hrs ago', category: 'ML_PRICE' },
  ])
    ...
    <header className="sticky top-0 bg-white border-b border-slate-200 py-3.5 px-6 z-20 flex items-center justify-between shadow-sm">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-[10px] text-slate-500 hidden sm:block">
            FreightIQ AI Automated Dispatch & Intelligence Risk Network
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3.5">
        ...
        <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 bg-emerald-50 text-emerald-700 border border-emerald-150 rounded-full text-[10px] font-bold tracking-wide uppercase">
          <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" /> Live SLA & Risk Monitored
        </div>
          ...
          {notificationsOpen && (
            <div className="absolute right-0 mt-2 w-80 bg-white border border-slate-200 rounded-2xl shadow-xl py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
              <div className="px-4 py-2 border-b border-slate-100 flex items-center justify-between">
                <span className="text-xs font-extrabold text-slate-850">Intelligence Alerts</span>
                <span className="text-[10px] text-blue-500 cursor-pointer font-semibold" onClick={() => setNotifications([])}>Clear</span>
              </div>
