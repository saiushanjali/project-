...
function Hero() {
  ...
  const stats = [
    { icon: <Zap className="w-5 h-5 text-primary-600" />, value: "99.4%", label: "On-Time Dispatch" },
    { icon: <Clock className="w-5 h-5 text-primary-600" />, value: "< 5s", label: "Instant Quote SLA" },
    { icon: <ShieldCheck className="w-5 h-5 text-primary-600" />, value: "$450M+", label: "Freight Managed" },
  ];

  return (
    <section
      id="home"
      className="relative bg-slate-50 text-slate-900 min-h-screen pt-28 pb-16 flex items-center overflow-hidden bg-grid-pattern"
    >
      {/* Glow Effects */}
      ...
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_40%,#f8fafc_100%)] pointer-events-none" />
        ...
        <div className="grid lg:grid-cols-12 gap-12 items-center">
          ...
          >
            ...
            >
              ...
              <span className="gradient-text-blue">Quote Generation</span>
            </motion.h1>
            ...
            >
              ...
              <a
                href="#features"
                className="w-full sm:w-auto bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200/80 hover:border-slate-300 font-semibold px-8 py-4 rounded-2xl text-center backdrop-blur-sm transition-colors"
              >
                Explore Features
              </a>
            </motion.div>
            ...
            <motion.div
              variants={itemVariants}
              className="grid grid-cols-3 gap-6 sm:gap-8 pt-8 border-t border-slate-200 w-full"
            >
              {stats.map((stat, i) => (
